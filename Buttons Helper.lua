...
function check_update()
    msg("Ïðîâåðêà íàëè÷èÿ îáíîâëåíèé...")
    ...
    if response and response ~= currentVersion then
        msg("Ó âàñ íå àêòóàëüíàÿ âåðñèÿ! Äëÿ îáíîâëåíèÿ ïåðåéäèòå âî âêëàäêó: Èíôîðìàöèÿ")
    else
        msg("Ó âàñ àêòóàëüíàÿ âåðñèÿ ñêðèïòà.")
    end
end
local function updateScript(scriptUrl, scriptPath)
    ...
    if response and response ~= currentVersion then
        msg("Äîñòóïíà íîâàÿ âåðñèÿ ñêðèïòà! Îáíîâëåíèå...")
        
        local success = downloadFile(scriptUrl, scriptPath)
        if success then
            msg("Ñêðèïò óñïåøíî îáíîâëåí.")
            thisScript():reload()
        else
            msg("Íå óäàëîñü îáíîâèòü ñêðèïò.")
        end
    else
        msg("Ñêðèïò óæå ÿâëÿåòñÿ ïîñëåäíåé âåðñèåé.")
    end
end
...
imgui.OnFrame(function() return WinState2[0] end, function(player)
    ...
    imgui.Begin('##2Window', WinState2, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoTitleBar + imgui.WindowFlags.AlwaysAutoResize) 
        ...
        if ini.knopa.pochinka then
            if imgui.Button(fa.CAR_WRENCH .. u8' Ïî÷èíèòü') then 
                sampSendChat('/repcar')
            end
        end
        ...
        if ini.knopa.dveri then
         ...
         if ini.knopa.rejim then
            if imgui.Button(fa.GAMEPAD .. u8' Ðåæèì') then 
                sampSendChat('/style')
            end
        end
        ...
        if ini.knopa.lek then
            if imgui.Button(fa.PILLS .. u8' Àïòå÷êà') then 
                sampSendChat('/usemed')
            end
        end
        
        if ini.knopa.arm then
            if imgui.Button(fa.SHIELD .. u8' Áðîíèê') then 
                sampSendChat('/armour')
            end
        end
    imgui.End()
...
msg("Çàãðóçêà õåëïåðà ïðîøëà óñïåøíî!")
show_arz_notify('info', 'Buttons', "Çàãðóçêà õåëïåðà ïðîøëà óñïåøíî! Àêòèâàöèÿ: /but", 3000)
print(' Çàãðóçêà õåëïåðà ïðîøëà óñïåøíî!')
msg("×òîá îòêðûòü ìåíþ õåëïåðà ââåäèòå êîìàíäó {009EFF}/but")
check_update()
...
imgui.OnFrame(function() return ButtonsM[0] end, function(player)
    ...
    if imgui.BeginTabBar('Tabs') then	           										  
        if imgui.BeginTabItem(fa.GEAR .. u8' Íàñòðîéêè êíîïîê') then
                ...
                if imgui.Checkbox(u8'Îòîáðàæåíèå êíîïêè [ Àïòå÷êà ] ',lek) then
                    ini.knopa.lek = lek[0]
                    cfg_save() 
                end
                
                if imgui.Checkbox(u8'Îòîáðàæåíèå êíîïêè [ Áðîíèê ] ',arm) then
                    ini.knopa.arm = arm[0]
                    cfg_save() 
                end
    ...
    if imgui.BeginTabItem(fa.INFO .. u8' Èíôîðìàöèÿ') then
    
    imgui.Text(fa.CIRCLE_USER..u8" Ðåäàêòîð äàííîãî õåëïåðà: Lion")
				imgui.Separator()
    ...
    updateScript(lmUrl, lmPath)
				...
				imgui.Separator()
				imgui.Text(fa.HEADSET..u8" Êàíàë ðåäàêòîðà õåëïåðà:")
				imgui.SameLine()
				if imgui.SmallButton('Telegram') then
					openLink('https://t.me/luamastery')
				end
				imgui.Separator()
				imgui.Text(fa.GLOBE..u8" Îñíîâà õåëïåðà íà BlastHack:")
				imgui.SameLine()
				if imgui.SmallButton(u8'Ïåðåéòè') then
          ...
          apply_n_t()
            ...
            imgui.Separator()
            imgui.Separator()
	imgui.CenterText(fa.CODE .. u8' Êîìàíäû')
	imgui.Text(fa.CALCULATOR .. u8' /calc - Êàëüêóëÿòîð')
                
						imgui.Separator()  
				      imgui.Separator()
	if imgui.Button(fa.ROTATE .. u8" Ïåðåçàãðóçèòü") then script_reload() end
	imgui.SameLine()
	if imgui.Button(fa.XMARK .. u8" Âûãðóçèòü") then script_unload() end
...
show_arz_notify('info', 'Buttons Helper', "Ïåðåçàãðóçêà....!", 500)
wait(0)
thisScript():reload()
end)
end
...
lua_thread.create(function()
show_arz_notify('info', 'Buttons Helper', "Âûêëþ÷åíèå....!", 500)
wait(0)
thisScript():unload()
end)
end
